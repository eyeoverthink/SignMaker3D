 ...
 * Generates complete holographic panel package with all STL files and documentation
 */
export async function generateHolographicPanel(settings: HolographicPanelSettings) {
  const { STLExporter } = await import('three/examples/jsm/exporters/STLExporter.js');
  const exporter = new STLExporter();
  
  // Generate all layers
  const layerSTLs: string[] = [];
  for (const layerConfig of settings.layers) {
    const layer = generateLayer(settings, layerConfig);
    const layerSTL = exporter.parse(layer, { binary: false });
    layerSTLs.push(layerSTL);
  }
  
  // Generate frame
  const frame = generateFrame(settings);
  const frameSTL = exporter.parse(frame, { binary: false });
  
  // Generate LED channel
  const ledChannel = generateLEDChannel(settings);
  const ledChannelSTL = exporter.parse(ledChannel, { binary: false });
  
  // Generate spacer clips
  const spacerClips = generateSpacerClips(settings);
  const spacerClipSTLs: string[] = [];
  for (let i = 0; i < settings.spacerClipCount * 4; i++) {
    spacerClipSTLs.push(exporter.parse(spacerClips, { binary: false }));
  }
  
  // Generate documentation
  const assemblyInstructions = generateHolographicPanelInstructions(settings);
  const bom = generateHolographicPanelBOM(settings);
  
  let wiringDiagram = '# Wiring Diagram - Holographic Panel\n\n';
  wiringDiagram += `## LED Connection (${settings.ledType})\n\n`;
  
  if (settings.ledType === 'ws2812b') {
    const ledCount = Math.ceil((settings.panelWidth + settings.panelHeight) * 2 / 16.67);
    const current = (ledCount * 0.06).toFixed(1);
    wiringDiagram += `### WS2812B Addressable LEDs
- **Data Pin**: Arduino D6 → LED Strip DIN
- **Power**: 5V → LED Strip VCC
- **Ground**: GND → LED Strip GND
- **Resistor**: 220Ω between Arduino D6 and LED Strip DIN

### Power Requirements:
- LED Count: ~${ledCount} LEDs
- Current: ~${current}A at full brightness
- Power Supply: 5V, 2A recommended\n\n`;
  } else if (settings.ledType === 'simple_5v') {
    const stripLength = Math.ceil((settings.panelWidth + settings.panelHeight) * 2 / 1000);
    wiringDiagram += `### Simple 5V LED Strip
- **Power**: 5V → LED Strip +
- **Ground**: GND → LED Strip -
- **Optional**: PWM dimmer for brightness control

### Power Requirements:
- Strip Length: ${stripLength}m
- Current: ~0.5A
- Power Supply: 5V, 1A\n\n`;
  } else {
    wiringDiagram += '### No LEDs\nThis panel uses ambient light only.\n\n';
  }
  
  wiringDiagram += `## Installation:
1. Route LED strip around panel perimeter
